...
CREATE_APPOINTMENT() {
  ...
  INSERT_APPOINTMENT=$($PSQL "INSERT INTO appointments (customer_id, service_id, time) VALUES ($CUSTOMER_ID, $SERVICE_SELECTION, '$SERVICE_TIME')")
...
PHONE_NUMBER=$($PSQL "SELECT customer_id, name FROM customers WHERE phone = '$CUSTOMER_PHONE'")
...
LIST_SERVICES(){
    ...
    SERVICE_SELECTION=$($PSQL "SELECT service_id FROM services WHERE service_id = $SERVICE_ID_SELECTED")
